...
def date(fully_merged_df,date_input):
    """
    Find the summary stock data for a date that the user chooses.

    Args:
        fully_merged_df (pandas.DataFrame): The merged DataFrame of the two stocks' data.
        date_input (str): The date that the user inputted
    """
    grouped_df = fully_merged_df.groupby('Date')
    print(grouped_df.get_group(date_input), "\n")
...
def main():
    ...
    while True:
        try:    
            ...
            while True:
                try:
                    date_input = input ('Enter a date between 2022-06-10 to 2023-06-09: (in format yyyy-mm-dd): ')
                    date_tokens = date_input.split('-')
                    year = int(date_tokens[0])
                    month = int(date_tokens[1])
                    day = int(date_tokens[2])
                    datetime_input = datetime.date(year, month, day)
                    if datetime_input > datetime.date(2022, 6, 9) and datetime_input < datetime.date(2023, 6, 9):
                        try:
                            print('Stock data for the date: ' + date_input)
                            date(fully_merged_df,date_input)
                            break
                        except KeyError:
                            print('The stock market was closed for ' + date_input)
                    else:
                        raise KeyError
                except KeyError:
